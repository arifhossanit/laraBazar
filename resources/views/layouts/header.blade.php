...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
  <div class="container px-4 px-lg-5">
    ...
    <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
      ...
      <form class="d-flex mx-5 input-group" action="{{route('search')}}" method="GET">
      ...
      <div class="d-flex">
        <a href="{{route('cartlist')}}" class="btn text-warning"><i class="fab fa-opencart"></i> Cart({{$total}})</a>
      </div>
      <div class="d-flex nav-item dropdown">
        ...
        @else
        <a class="nav-link text-white" href="{{route('user.login.page')}}">Login</a>
        @endif
      </div>
      <div class="d-flex nav-item">
        @if(!Session::get('user'))
        <a class="nav-link text-white" href="{{route('user.register.page')}}">Register</a>
        @endif
      </div>
    </div>
  </div>
